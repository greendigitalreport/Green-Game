...
export default function App() {
  ...
  const joinGame = async (e: React.FormEvent) => {
    e.preventDefault();
    const q = query(collection(db, 'games'), where('code', '==', gameCode), where('status', '==', 'waiting'));
  ...
  return (
    <div className="font-sans antialiased text-gray-900">
      <AnimatePresence mode="wait">
        ...
        {view === 'admin-login' && <motion.div key="admin-login" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><AdminLoginView /></motion.div>}
        {view === 'admin' && <motion.div key="admin" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><AdminDashboard /></motion.div>}
        {view === 'player-join' && <motion.div key="player-join" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><PlayerJoinView /></motion.div>}
        ...
        {view === 'results' && <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><ResultsView /></motion.div>}
      </AnimatePresence>
    </div>
  );
}
